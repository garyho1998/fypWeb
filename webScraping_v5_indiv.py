...
class RecursiveScraper:
    ...
    def scrapeBFS(self, mainurl):
        ...
        while self.urlQueue and num_visited_node<self.maxNode and num_words<self.maxWords:
            ...
            try:
                ...
                content = self.boilerpipe(self.browser.page_source)
    ...
    def boilerpipe(self,page_source):
        if page_source is None:
            return ""
        try:
            extractor = Extractor(extractor='KeepEverythingExtractor', html=page_source)
        except:
            print("extract error")
            return ""
...
def start(url):
    try:
        ...
        for url in df_urls:
            ...
            fileName = "WebScraping/" + urlparse(url).netloc.replace('www.','').replace('/','-').replace(":","_").replace('.','-') + "_test.txt"     

            print("website", url)
            rscraper = RecursiveScraper(maxWords=500000, maxNode=300)
            rscraper.scrapeBFS(mainurl = url)
            mydict = {k: str(v).encode("utf-8") for k,v in rscraper.mainContent_dict.items()}

            with open(fileName, "w", encoding="utf-8") as f:
                for (key, value) in rscraper.mainContent_dict.items():
...
if __name__ == '__main__':
    ...
    urls = "https://www.acxhk.com/product"
    start(urls)
